...
export const markLessonCompleted = async (userId, lessonId, score = 0, stars = 0, signsLearned = 0) => {
    try {
        ...
        // Calculate today's progress (goal: 2 lessons per day)
        const lastPracticeDate = currentData.progress?.lastPracticeDate?.toDate();
        const now = new Date();
        const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
        
        let lessonsCompletedToday = 1; // This lesson counts as 1
        ...
        if (lastPracticeDate) {
            ...
            if (lastPractice.getTime() === today.getTime()) {
                // Same day, count existing completions
                lessonsCompletedToday = (currentData.progress?.lessonsCompleted || 0) + 1;
            }
        }
        
        const dailyGoal = currentData.settings?.dailyGoal || 20; // This is min practice time, but we'll use 2 lessons
        const todayProgress = Math.min((lessonsCompletedToday / 2) * 100, 100);
